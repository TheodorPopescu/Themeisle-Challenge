...
async function insertUsers() {
  ...
  // Insert specific test users first
  const specificUsers = await db.insert(schema.usersTable).values([
    {
      username: "admin",
  ...
  if (specificUsers.length !== 2) {
    throw new Error("Failed to create specific test users");
  }

  const [adminUser, user1] = specificUsers;
  ...
  const seededUsers: SeededUser[] = [
    // Include specific test users
    {
      id: adminUser!.id,
      username: adminUser!.username,
      email: adminUser!.email,
      password: "password123",
      remainingBalance: adminUser!.balance || 10000,
    },
    {
      id: user1!.id,
      ...
      email: user1!.email,
      password: "password123",
      remainingBalance: user1!.balance || 1000,
    },
...
async function insertMarkets(users: SeededUser[]) {
  ...
  for (let index = 0; index < MARKET_COUNT; index++) {
    ...
    if (!createdMarket) {
      throw new Error(`Failed to create market at index ${index}`);
    }
